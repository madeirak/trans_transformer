...
with tf.Session() as sess:
    ...
    if os.path.exists('model/model.meta'):
        print('loading  model...')
        saver.restore(sess, 'model')#该save_path参数通常是先前从save()调用或调用返回的值 latest_checkpoint()
    ...
    saver.save(sess, 'model/model_%d' % epoch)
    writer.close()
